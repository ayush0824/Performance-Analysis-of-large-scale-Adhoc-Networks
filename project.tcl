...
####  Setting The Distance Variables
      ...
      #strength of transmitted signal vs distance 
...
#RX-THRESH-reciever sensitivity transmission range
# CSTHRESH-carrier sense transmission range
...
# opt(seed)=seeding/generating random number in same way as we do in back-off algorithm at mac layer.
###  Setting The Initial Positions of Nodes
      ...
      $ns_ at 0.75 "$n19 setdest 437.0 699.0 20.0"
      $ns_ at 0.20 "$n20 setdest 159.0 722.0 50.0"
      $ns_ at 0.25 "$n21 setdest 900.0 250.0 50.0"
      $ns_ at 0.20 "$n22 setdest 839.0 144.0 50.0"
...
#Loss Monitor is attached with receiver node.Loss Monitor objects trace #out the lost packets, and received packets details.Packet Loss is #measured by accessing the loss monitor object.
      $udp0 set fid_ 1   
    #id field is  used for specifying stream color for the NAM display 
      set cbr0 [$udp0 attach-app Traffic/CBR]
      $cbr0 set packetSize_ 1000    
      $cbr0 set interopt_ .07
      $ns_ at 0.0 "$cbr0 start"
      $ns_ at 4.2 "$cbr0 stop"
      ...
      $ns_ at 0.4 "$cbr2 start"
      $ns_ at 6.1 "$cbr2 stop"
...
set opt(netif)          Phy/WirelessPhy            ;# network interface type 
set opt(mac)            Mac/802_11                 ;# MAC type 
set opt(ifq)            CMUPriQueue                ;# interface queue type this type of queue sends packt accord to priority(manages)
set opt(ll)             LL                         ;# link layer type 
set opt(ant)            Antenna/OmniAntenna        ;# antenna model 
set opt(ifqlen)         50                         ;# max packet in ifq 
